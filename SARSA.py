...
if __name__ == "__main__":
    ...
    for i in range(EPISODES):
        ...
        while not done:
            ...
            if np.random.random() > EPSILON:
                new_action = np.argmax((Q_table[new_state, 0], Q_table[new_state, 1], Q_table[new_state, 2], Q_table[new_state, 3]))
            else:
                new_action = env.action_space.sample()
            current_q = Q_table[state, action]
            new_q = Q_table[new_state, new_action]
            max_future_q = episode_reward + DISCOUNT_FACTOR * new_q - current_q

            Q_table[state, action] = current_q + LEARNING_RATE * max_future_q
            action = new_action
            state = new_state
            if done:
                done_q = episode_reward - new_q
                Q_table[new_state, new_action] = new_q + LEARNING_RATE * done_q
